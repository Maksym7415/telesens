  ...
  const handleClickGenre = (data, catId) => {
   ...
   return (
     <div className='root-page'>
       <Genres data= {props.data && props.data.map(genre => !genre.parentCatId?  <li className= {current === genre.contentCatId ? 'active':''} onClick = {()=> handleClickGenre(props.data, genre.contentCatId)} key= {genre.contentCatId}>{genre.catName}</li> : '')}/>

       <SubCats data= {props.data && current !== null && props.data.map(item => item.parentCatId && item.parentCatId === current? <li onClick = {() =>  setSubCat(item.contentCatId)} className= {subCat === item.contentCatId ? 'active':''} key={item.contentCatId}>{item.catName}</li>: '')}/>
